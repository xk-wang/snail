...
#ifndef SPRINGSNAIL_PROCESSPOLL_H
#define SPRINGSNAIL_PROCESSPOLL_H
...
static void addsig(int sig, void(handler)(int), bool restart=true){
    ...
    sigfillset(&sa.sa_mask); //进入到信号处理函数之前需要屏蔽的信号
        ...
        : m_listenfd(listenfd), m_process_number(process_number), m_idx(-1), m_stop(false)
{
    ...
    for(int i=0;i<process_number;++i){
        // 通过父子进程之间的资源拷贝实现了进程和子进程之间的管道通信机制
    ...
    while (!m_stop) {
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if (sockfd == m_listenfd) {
                ...
                if(ret<=0) continue;
                // 这里的子进程感觉正常情况是不会停止的，因为子进程也是一个无限循环，除非子进程
                // 自己出问题或者被杀死才会出现子进程结束的情况。现在是每个逻辑服务器配置一个子进程
                // 每个子进程管理一定数量的链接资源，通过mgr类来管理
                for(int i=0;i<ret;++i){
                    switch(signals[i]){
                        case SIGCHLD:{
                            pid_t pid;
            ...
            }else if(events[i].events & EPOLLIN){ //这个需要去读的数据一定是来自子进程
                int busy_ratio = 0;
        ...
        for(int i=0;i<number;++i){
            ...
            if(sockfd==pipefd_read&&events[i].events&EPOLLIN){
                ...
                if(ret<0&&errno!=EAGAIN || ret ==0 ) continue;
                    ...
                    C* conn = manager->pick_conn(connfd); //分配连接资源
                    if(!conn){
                    ...
                    for(int i=0;i<ret;++i){
                        switch(signals[i]){
                            case SIGCHLD:{
                                pid_t pid;
                                int stat;
                                while(pid=waitpid(-1, &stat, WNOHANG)) continue;
                                break;
                            }
                            case SIGTERM:
                            case SIGINT:{
...
// 向父进程告知繁忙程度，每次接收连接和连接关闭的时候
// 不是那么的精确，因为还没回收来的资源是没利用的
template<typename C, typename H, typename M>
...
void processpool<C, H, M>::setup_sig_pipe(){
    ...
    addsig(SIGPIPE, SIG_IGN); // 忽略这个信号 防止对方关闭连接时进程异常退出
    // 这样我们通过读socket来判断对方关闭了连接ret=-1 errno=SIGPIPE
}

#endif //SPRINGSNAIL_PROCESSPOLL_H
